...
include_once ('./includes/connection_includes.php');
    ...
    $information = '';
    $withdrawalInvestorCode  = filter_var(trim(mysqli_real_escape_string($connectionResult, $_GET['investor'])), FILTER_SANITIZE_STRING);
    $withdrawalCode  = filter_var(trim(mysqli_real_escape_string($connectionResult, $_GET['withdrawalCode'])), FILTER_SANITIZE_STRING);
    
    
    if ($investorSession == true) {
        $transWallet = filter_var(trim(mysqli_real_escape_string($connectionResult, $_GET['tranMs'])), FILTER_SANITIZE_STRING);
        // $transId = $transWallet;
        
        if (isset($_POST['approveBtnId'])) {
            $transId = filter_var(trim(mysqli_real_escape_string($connectionResult, $_POST['approveWithdrawalId'])), FILTER_SANITIZE_STRING);

            $approve = new PendingWithdrawalClass();

            $result = $approve ->approveWithdrawal($connectionResult, $transId);
            $information = $result;
        }
?>
<!DOCTYPE html>
...
<body>
    <nav id="navbar" class="navbar navbar-light fixed-top justify-content-between" style="background-color:  #000; margin-bottom: 60px;">
        <a class="navbar-brand" href="#" style="color: #fff;">Approve Withdrawal</a>
            ...
            <a href="admin_dashboard.php?admin=<?php echo $investorId?>&investor=<?php echo $withdrawalInvestorCode;?>&withdrawalCode=<?php echo $withdrawalCode;?>" class="d-lg-block d-sm-block d-md-block">
        ...
        <div class="container-fluid">
            <div class="row" style="width: 100%; height: 1000px;">
                ...
                <div class="col-xl-12 col-sm-12 col-md-12" style="margin-top: 70px;">
                    <div class="row">
                        <p class="text-left ml-4" style="text-transform: Uppercase; font-weight: bold;">Approve Withdrawal</p>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-xl-12 col-sm-12 col-md-12" style="background-color: #808090; border-radius: 10px; height: 70%; margin-left: 15px;">
                            <form class="row d-inline" method="post" action="approve_withdrawal.php?admin=<?php echo $investorId?>&investor=<?php echo $withdrawalInvestorCode;?>&withdrawalCode=<?php echo $withdrawalCode;?>&tranMs=<?php echo $transWallet;?>">
                                ...
                                <div class="row" style="margin-bottom: 10px; padding-left: 10px; padding-top: 10px; padding-right: 10px;">
                                    <p class="col"><label for="removeMoneyId" style="margin-right: 50px;">Transition Id: </label></p>
                                    <!-- <input type="text" name="approveWithdrawalId" id="approveWithdrawalId" style="width: 400px;" class="col" value="<?php echo $transId;?>" disabled> -->
                                    <input type="text" name="approveWithdrawalId" id="approveWithdrawalId" style="width: 400px;" class="col">
                                </div>
                                <div class="text-right">
                                    <input type="submit" value="Approve" class="btn" name="approveBtnId">
                                </div>
                            </form>
                        </div>
                    </div>
