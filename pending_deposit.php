...
include_once ('./includes/connection_includes.php');
    ...
    $pendingDeposit = new PendingDepositClass();
    if (isset($_POST['approveId'])) {
        $pendingDeposit->approveDeposit($connectionResult);
    }

    if (isset($_POST['declineId'])) {
        $pendingDeposit->declineDeposit($connectionResult);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    <nav id="navbar" class="navbar navbar-light fixed-top justify-content-between" style="background-color:  #000; margin-bottom: 60px;">
        <a class="navbar-brand" href="#" style="color: #fff;">Dashboard</a>
            ...
            <a href="admin_dashboard.php?admin=<?php echo $investorId;?>" class="d-lg-bloxk d-sm-block d-md-block">
        ...
        <div class="container-fluid">
            <div class="row" style="width: 100%; height: 1000px;">
                ...
                <div class="col-md-12 col-sm-12 col-xl-12" style="margin-top: 70px;">
                    <div class="row">
                        <p style="text-transform: uppercase; margin-left: 30px; margin-bottom: 20px; font-weight: 700; color:#000000;">Here below are the pending deposits to approve or not:</p>
                    </div>
                    <div class="row">
                        <table class="table" style="margin-left: 30px; margin-right: 30px;">
                            ...
                            <?php
                                ...
                                $selectDeposit = "SELECT `username`, `transaction_code`, `transaction_plan`, `transaction_date`, `transaction_payment_method`, `investor_referral_code`, `amount`, `transId`  FROM `deposittransaction` WHERE `is_transaction_approved` = '0' AND `saved_transaction` = '1' AND `isDepositDecline` = '0' ORDER BY `transaction_id` DESC";
                                    ...
                                    if ($numRows == 0) {
                                        ...
                                        while ($a = mysqli_fetch_assoc($querySelectDeposit)) {
                                            ...
                                            <tr >
                                                ...
                                                <form action="pending_deposit.php?admin=<?php echo $isIdTrue?>&investor=<?php echo $transactionInvestorCode;?>&depositCode=<?php echo $transactionCode;?>" method="post" class="d-inline">
                                                    <input type="submit" name="approveId" value="Approve" class="btn" style="background-color: rgb(40, 97, 7); color: #fff; margin-bottom: 10px; width: 100px;">
                                                    <input type="submit" name="declineId" value="Decline" class="btn" style="background-color: #dc3545; color: #fff; margin-bottom: 10px; width: 100px;">
                                                </form>
                                                </td>
                                            </tr>
                                            <?php
